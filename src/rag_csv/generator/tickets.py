...
from rag_csv.config.settings import QdrantConfig, EmbeddingConfig, DataConfig, GeneratorConfig, LoggingConfig, OllamaConfig
from rag_csv.config.logging import get_logger
from rag_csv.utils import metrics as metrics_utils
...
OLLAMA_HOST = _ollama_cfg.url
OLLAMA_MODEL_INCIDENTS = _data_cfg.model_incidents

# Gesamtanzahl zu generierender Tickets
TOTAL_TICKETS = _data_cfg.total_tickets

# Wie viele Tickets pro Ollama-Call generiert werden sollen
TICKETS_PER_CALL = _data_cfg.tickets_per_call

# Pfad der Ausgabedatei
OUTPUT_CSV_PATH = _gen_cfg.output_csv_path

# Ausgabedatei
OUTPUT_CSV_FILENAME = os.path.join(OUTPUT_CSV_PATH, f"synthetic_incidents_llm_{OLLAMA_MODEL_INCIDENTS}.csv")
...
class TicketGenerator:
    ...
    def _call_ollama(self, prompt: str) -> tuple[str, int, int, float]:
        ...
        payload = {
            ...
            "stream": False,
            "options": {
                "temperature": _gen_cfg.generator_temperature,
                "top_p": _gen_cfg.generator_top_p, 
                "num_ctx": _gen_cfg.generator_ctx_tokens,
                "repeat_penalty": _gen_cfg.generator_repeat_penalty,
